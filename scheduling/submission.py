...
def create_nqueens_csp(n: int = 8) -> CSP:
    ...
    variables = [f'x{i}' for i in range(1, n+1)]
    for index, variable in enumerate(variables):
        csp.add_variable(variable, [(index, i) for i in range(n)])
        ...
        for quarter in self.profile.quarters:
            ...
            for request in self.profile.requests:
                ...
                for course in courses:
                    ...
                    domain = [0] 
                    domain.extend(range(min_course_unit, max_course_unit+1))
            ...
            quarterly_units = create_sum_variable(csp,'sum_var_'+quarter,variables,self.profile.maxUnits)
