...
def mutate_sentences(sentence: str) -> List[str]:
    ...
    def dfs(start_word, cur_sentence):
        if len(cur_sentence.split()) == len(words):
            result.append(cur_sentence)
            return
        
        for next_word in pair_dict[start_word]:
            dfs(next_word, cur_sentence + " " + next_word)
...
def find_nonsingleton_words(text: str) -> Set[str]:
    ...
    return {text for text, count in word_count.items() if count > 1}
    # END_YOUR_CODE
