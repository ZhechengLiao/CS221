...
class ShortestPathProblem(SearchProblem):
    ...
    def successorsAndCosts(self, state: State) -> List[Tuple[str, State, float]]:
        ...
        if not self.isEnd(state):
            for nextLocation in self.cityMap.distances[state.location]:
                cost = self.cityMap.distances[state.location][nextLocation]
                newState = State(location=nextLocation)
                result.append((nextLocation, newState, cost))
            ...
            def successorsAndCosts(self, state: State) -> List[Tuple[str, State, float]]:
                ...
                if state.location == "Special END State":
                    for endLocation in cityMap.geoLocations.keys():
                        if endTag in cityMap.tags[endLocation]: result.append((endLocation, State(endLocation, None), 0))
                else:
                    for nextLocation, cost in cityMap.distances[state.location].items(): result.append((nextLocation, State(nextLocation, None), cost))
