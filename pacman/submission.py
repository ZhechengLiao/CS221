...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState) -> str:
        ...
        def getScore(state, d, alpha, beta, agentIndex):
            legalActions = state.getLegalActions(agentIndex)
            if d == 0: 
                return [self.evaluationFunction(state), None]
            elif state.isWin() or state.isLose() or len(legalActions) == 0:
                ...
                if agentIndex == state.getNumAgents() - 1:
                    ...
                    nextAgent = agentIndex + 1
                    depth = d 
                # different agent play differently
                if agentIndex == 0:
                    value = float("-inf")
                    scores = []
                    action = None
                    for act in legalActions:
                        score = getScore(state.generateSuccessor(agentIndex, act), depth, alpha, beta, nextAgent) 
                        if score[0] > value:
                            value = score[0]
                            action = act
                    ...
                    for act in legalActions:
                        ...
                        if beta <= alpha:
                            break
                    return [value, action]
        score, action = getScore(gameState, self.depth, float("-inf"), float("inf"), self.index)  
        return action
    ...
    def getAction(self, gameState: GameState) -> str:
        ...
        def getScore(state, agentIndex, d):
            ...
            if d == 0:
                ...
                nextScores = [getScore(state.generateSuccessor(agentIndex, action), nextAgent, depth)[0] for action in legalActions]
                if agentIndex == 0:
                    max_index, max_score = max(enumerate(nextScores), key=lambda p: p[1])
                    return [max_score, legalActions[max_index]]
                else:
