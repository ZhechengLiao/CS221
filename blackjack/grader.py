...
############################################################
# Problem 2
grader.add_manual_part('2a', 0, description="Written question: efficient value iteration for acyclic MDP")

grader.add_manual_part('2b', 4, description="Written question: define new MDP solver for discounts < 1")
...
def test3a0():
    ...
    mdp4 = submission.BlackjackMDP(cardValues=[2, 3, 4], multiplicity=2,
                                   threshold=1, peekCost=1)
    mdp4_startState = mdp4.startState()
    ...
    tests = [
        ...
        ([((11, 0, (1, 0)), 1, -1)], mdp2, preEmptyState, 'Peek'),
        # Not allowed to peek twice in a row
        ([], mdp2, afterPeekState, 'Peek'),
        # Cannot peak because no cards left
        ([], mdp2, (12, None, (0, 0)), 'Peek'),
        ...
        # Tests with MDP4
        # Go bust immediately because of low threshold, regardless of card taken
        ([((2, None, None), 1/3, 0), ((3, None, None), 1/3, 0), ((4, None, None), 1/3, 0)], mdp4, mdp4_startState, 'Take')
    ]
...
grader.add_basic_part('3a-0-basic', test3a0, 5,
...
grader.add_hidden_part('3a-2-hidden', test3a2Hidden, 5,
...
grader.add_basic_part('4a-basic', test4a, 5, max_seconds=10,
...
grader.add_hidden_part('4a-hidden', test4aHidden, 3, max_seconds=3,
                       description="Hidden test for incorporateFeedback(). Run QLearningAlgorithm on smallMDP, then ensure that getQ returns reasonable value.")


def test4aHiddenExtra():
    smallMDP = submission.BlackjackMDP(cardValues=[1, 5], multiplicity=2, threshold=10, peekCost=1)
    mdp = smallMDP

    mdp.computeStates()
    rl = submission.QLearningAlgorithm(mdp.actions, mdp.discount(), submission.simpleFeatureExtractor, 0.2)
    util.simulate(mdp, rl, 50000)



grader.add_hidden_part('4a-hidden-extra', test4aHiddenExtra, 3, max_seconds=5,
                       description="Hidden test for incorporateFeedback(). Use submission.simpleFeatureExtractor; Run QLearningAlgorithm on smallMDP, then ensure that getQ returns reasonable value.")
...
grader.add_basic_part('4b-helper', run4bHelper, 0, max_seconds=60,
...
grader.add_basic_part('4c-basic', test4c, 5, max_seconds=10,
...
grader.add_manual_part('4d', 0,
                       description="Written question: reward comparison for applying policy to baseline and modified MDP")
...
def run4dHelper():
    submission.compare_changed_MDP(submission.originalMDP, submission.newThresholdMDP,
                                   submission.blackjackFeatureExtractor)


grader.add_basic_part('4d-helper', run4dHelper, 0, max_seconds=60,
...
grader.add_manual_part('5a', 2,
                       description="Written question: policy comparison for short time horizon MDP versus long time horizon MDP")
...
grader.add_basic_part('5a-helper', run5aHelper, 0, max_seconds=10,
                      description="Helper function to compare optimal policies over various time horizons.")  #

grader.add_manual_part('5b', 2, description="Written question: Ethical frameworks for making time horizon decisions")

grader.add_manual_part('5c', 2,
                       description="Written question: policy comparison for discounted versus non-discounted MDP")
...
grader.add_basic_part('5c-helper', run5cHelper, 0, max_seconds=10,
                      description="Helper function to compare optimal policies over various discounts.")  #

grader.add_manual_part('5d', 0, description="Written question: Exploring how optimal policies transfer across MDPs")
...
grader.add_basic_part('5d-helper', run5dHelper, 0, max_seconds=10,
                      description="Helper function for exploring how optimal policies transfer across MDPs.")  #
