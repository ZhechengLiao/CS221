...
class BlackjackMDP(util.MDP):
    ...
    def succAndProbReward(self, state: Tuple, action: str) -> List[Tuple]:
        ...
        result = [] # [(newState, prob, reward)]    

        if deckCardCounts is None:
            return []
        
        if action == 'Peek': # state, prob=1.0, reward=-peekCost
        ...
        elif action == 'Take': # state + num of card, prob=cardCount / sum(deckCardCounts), reward=0
            if nextCardIndexIfPeeked is not None:
                newCardValue = totalCardValueInHand + self.cardValues[nextCardIndexIfPeeked]
                newCardList = list(deckCardCounts)
                newCardList[nextCardIndexIfPeeked] -= 1
                if newCardValue > self.threshold or sum(newCardList) == 0: # bust or empty deck
                    return [((newCardValue, None, None), 1, 0 if newCardValue > self.threshold else newCardValue)]
                else:
                    return [((newCardValue, None, tuple(newCardList)), 1, 0)]

            else:
                for i, cardCount in enumerate(deckCardCounts):
                    if cardCount > 0:
                        newCardValue = totalCardValueInHand + self.cardValues[i]
                        newCardList = list(deckCardCounts)
                        newCardList[i] -= 1
                        prob = cardCount / sum(deckCardCounts)

                        if newCardValue > self.threshold or sum(newCardList) == 0: # bust or empty deck
                            result.append((((newCardValue, None, None), prob, 0 if newCardValue > self.threshold else newCardValue)))
                        else:
                            result.append((((newCardValue, None, tuple(newCardList)), prob, 0)))

        elif action == 'Quit': # action == Quit  
            newState = (totalCardValueInHand, None, None)
            result.append((newState, 1, totalCardValueInHand))
...
def blackjackFeatureExtractor(state: Tuple, action: str) -> List[tuple]:
    ...
    feature = []
    # total feature
    totalFeature = (('total', total, action), 1)
    feature.append(totalFeature)

    if counts is not None:
        # bitmask
        bitmask = tuple(int(card > 0) for card in counts)
        featureBitmask = (('bitmask', bitmask, action), 1)
        feature.append(featureBitmask)
        
        # Indicators for the action and the number of cards remaining with each face value. 
        countFeature = [((i,count,action), 1) for i, count in enumerate(counts)]
        feature.extend(countFeature)
    # END_YOUR_CODE
    return feature
